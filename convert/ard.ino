...
void loop() {
 String computerText = Serial.readStringUntil('@');
  ...
  String command = getValue(computerText, ' ',0);
    ...
    if (command == "يمين" || command == "Right" || command == "right"  ) {
      base.write(base_angle -= 20);
    }
    if (command == "يسار" || command == "Left" || command == "left"  ) {
     base.write(base_angle += 20);
    }

    if (command == "أعلى" ||command == "Top" || command == "top"  ) {
      shoulder.write(shoulder_angle -= 20);
    }

   if (command == "أسفل" || command == "Bottom"|| command == "bottom" ) {
     shoulder.write(shoulder_angle += 20);
    }
